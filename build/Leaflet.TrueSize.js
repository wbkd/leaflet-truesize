(function (global, factory) {
	typeof exports === 'object' && typeof module !== 'undefined' ? factory(require('leaflet'), require('@turf/helpers')) :
	typeof define === 'function' && define.amd ? define(['leaflet', '@turf/helpers'], factory) :
	(factory(global.L,global.helpers));
}(this, (function (L,helpers) { 'use strict';
 ...
 */
function radiansToLength$3(radians, units) {
...
function distance(from, to, options) {
    ...
    var dLat = degreesToRadians$3((coordinates2[1] - coordinates1[1]));
    var dLon = degreesToRadians$3((coordinates2[0] - coordinates1[0]));
    var lat1 = degreesToRadians$3(coordinates1[1]);
    var lat2 = degreesToRadians$3(coordinates2[1]);

    var a = Math.pow(Math.sin(dLat / 2), 2) +
          Math.pow(Math.sin(dLon / 2), 2) * Math.cos(lat1) * Math.cos(lat2);

    return radiansToLength$3(2 * Math.atan2(Math.sqrt(a), Math.sqrt(1 - a)), units);
}

/**
 * Earth Radius used with the Harvesine formula and approximates using a spherical (non-ellipsoid) Earth.
 */
var earthRadius$4 = 6371008.8;

/**
 * Unit of measurement factors using a spherical (non-ellipsoid) earth radius.
 */
var factors$4 = {
    meters: earthRadius$4,
    metres: earthRadius$4,
    millimeters: earthRadius$4 * 1000,
    millimetres: earthRadius$4 * 1000,
    centimeters: earthRadius$4 * 100,
    centimetres: earthRadius$4 * 100,
    kilometers: earthRadius$4 / 1000,
    kilometres: earthRadius$4 / 1000,
    miles: earthRadius$4 / 1609.344,
    nauticalmiles: earthRadius$4 / 1852,
    inches: earthRadius$4 * 39.370,
    yards: earthRadius$4 / 1.0936,
    feet: earthRadius$4 * 3.28084,
    radians: 1,
    degrees: earthRadius$4 / 111325,
};
 ...
 */
function lengthToRadians$4(distance, units) {
    if (distance === undefined || distance === null) throw new Error('distance is required');

    if (units && typeof units !== 'string') throw new Error('units must be a string');
    var factor = factors$4[units || 'kilometers'];
    if (!factor) throw new Error(units + ' units is invalid');
 ...
 * @returns {number} degrees between 0 and 360 degrees
 */
function radiansToDegrees$4(radians) {
    if (radians === null || radians === undefined) throw new Error('radians is required');
...
function destination(origin, distance, bearing, options) {
    ...
    var longitude1 = degreesToRadians$4(coordinates1[0]);
    var latitude1 = degreesToRadians$4(coordinates1[1]);
    var bearing_rad = degreesToRadians$4(bearing);
    var radians = lengthToRadians$4(distance, units);
    ...
    var lng = radiansToDegrees$4(longitude2);
    var lat = radiansToDegrees$4(latitude2);

    return point$4([lng, lat], properties);
}
...
L.TrueSize = L.Layer.extend({
  ...
  _addMarker: function _addMarker(layer, options) {
    var _this = this;
    ...
    this._dragMarker = L.marker(layer.getCenter(), { icon: dragIcon, draggable: true }).on('drag', function (evt) {
      return _this._onDrag(evt, layer);
    }).addTo(this._map);
  },
  _makeDraggable: function _makeDraggable(layer) {
    var _this2 = this;

    var draggable = new L.Draggable(layer._path);
    draggable.on('drag', function (evt) {
      return _this2._onDrag(evt, layer);
    }).enable();
  },
