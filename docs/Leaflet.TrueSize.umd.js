(function(global, factory) {
  ...
  var helpers = createCommonjsModule(function(module, exports) {
    ...
    function isNumber(num) {
      return (
        !isNaN(num) && num !== null && !Array.isArray(num) && !/^\s*$/.test(num)
      );
    }
  ...
  function getCoord(coord) {
    ...
    if (
      Array.isArray(coord) &&
      coord.length >= 2 &&
      coord[0].length === undefined &&
      coord[1].length === undefined
    )
      return coord;
    ...
    function lineEach(geojson, callback) {
      ...
      flattenEach(geojson, function(feature, featureIndex, multiFeatureIndex) {
        ...
        switch (type) {
          case 'LineString':
            if (
              callback(feature, featureIndex, multiFeatureIndex, 0, 0) === false
            )
              return false;
            break;
          case 'Polygon':
    ...
    function findSegment(geojson, options) {
      ...
      switch (geometry.type) {
        ...
        case 'MultiPolygon':
          ...
          if (segmentIndex < 0)
            segmentIndex =
              coords[multiFeatureIndex][geometryIndex].length -
              segmentIndex -
              1;
          return helpers.lineString(
            [
      ...
      switch (geometry.type) {
        ...
        case 'Polygon':
          ...
          return helpers.point(
            coords[geometryIndex][coordIndex],
            properties,
            options
          );
        case 'MultiLineString':
  ...
  L.TrueSize = L.Layer.extend({
    ...
    onAdd: function onAdd(map) {
      ...
      var centerCoords = this._currentLayer.getBounds().getCenter(); // wrap currentlayer into draggable layer
    ...
    _getLatLngFromPosition: function _getLatLngFromPosition(pos) {
      ...
      var _this$_map$containerP = this._map.containerPointToLatLng(
          posWithOffset
        ),
    ...
    _getBearingDistance: function _getBearingDistance(center) {
      ...
      if (this._isMultiPolygon()) {
        return this._currentLayer.feature.geometry.coordinates.map(function(
          coords
        ) {
